...
//
// 用法：
//   node scripts/tools/restore-project-from-orphan-db.mjs <projectId> "<书名>" <ownerUsername>
//   例：node scripts/tools/restore-project-from-orphan-db.mjs c980335a-... "空山雨后" user
//
...
if (!projectId || !name || !ownerUsername) {
  console.log('用法: node scripts/tools/restore-project-from-orphan-db.mjs <projectId> "<书名>" <ownerUsername>');
  process.exit(1);
}
...
console.log('');
console.log('下一步：重启 server 后确认书架能看到它，并跑一次 `node scripts/tools/clean-orphan-project-dbs.mjs --list`（应不再列为孤儿）。');
