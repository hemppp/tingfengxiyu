...
//   node scripts/tools/run-chapters.mjs --new --from 1 --count 30
//   # 续跑（脚本会打印账号与项目 id）
//   node scripts/tools/run-chapters.mjs --user probeXXX --project <projectId> --from 6 --count 25
//
...
if (!isNew && (!USERNAME || !projectId)) {
  console.log('用法：\n  node scripts/tools/run-chapters.mjs --new --from 1 --count 30\n  node scripts/tools/run-chapters.mjs --user <username> --project <projectId> --from 6 --count 25');
  process.exit(1);
}
...
say(`日志：${LOG}`);
console.log(`\n续跑命令：node scripts/tools/run-chapters.mjs --user ${USERNAME} --project ${projectId} --from <下一章> --count <剩余章数>`);
console.log(`清理命令：node scripts/verify/verify-autowrite-cleanup.mjs ${USERNAME} ${projectId}`);
