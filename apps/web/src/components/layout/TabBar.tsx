...
interface TabBarProps {
  ...
  /** 'main' = 主编辑器组（只有固定的正文项）；'side' = 侧编辑器组（可开可关的看板） */
  variant?: 'main' | 'side';
...
export function TabBar({
  ...
  onOpenQuick, bodySubtitle, onMove,
}: TabBarProps) {
  ...
  return (
    ...
    >
      {/* ── 组 1：唯一的固定标签「正文」（VSCode 的 pinned 标签 = 名字前一个图钉） ── */}
      {isMain && (
        <div
          role="tab"
